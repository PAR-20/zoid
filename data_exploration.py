...
for path in dataset_paths:
    ...
    if not os.listdir(path):
        ...
        os.makedirs(os.path.join(path, 'class2'), exist_ok=True)

# Function to explore image dataset
def explore_image_dataset(dataset_path):
    if not os.path.exists(dataset_path):
        print(f"Le répertoire {dataset_path} n'existe pas. Création du répertoire...")
    ...
    # Vérifier si le répertoire contient des classes
    if not classes:
        print(f"Aucune classe trouvée dans {dataset_path}. Utilisez la fonction download_dataset_from_huggingface pour télécharger un dataset.")
        return {}, []

    # Analyser les classes et les images
    for class_name in classes:
        ...
        for img_name in images[:5]:
            ...
            try:
                ...
                print(f"Erreur lors de l'ouverture de l'image {img_path}: {str(e)}")

    print(f"Dataset: {dataset_path}")
    print(f"Classes: {classes}")
    print(f"Class distribution: {class_distribution}")
    print(f"Sample image sizes: {image_sizes[:5] if image_sizes else 'Aucune image trouvée'}")
    print("-" * 50)

    # Ne pas essayer de visualiser si aucune donnée n'est disponible
...
def analyze_dataset_statistics(dataset_paths):
    ...
    if not valid_paths:
        print("Aucun dataset valide à analyser.")
        return

    if len(valid_paths) == 1:
        print(f"Un seul dataset valide ({os.path.basename(valid_paths[0])}), pas de comparaison nécessaire.")
        return
    ...
    dataset_names = [os.path.basename(path) for path in valid_paths]
...
def download_dataset_from_huggingface(dataset_name, save_dir='/Users/dalm1/Desktop/reroll/Progra/par20/downloaded_dataset'):
    ...
    try:
        ...
        # Télécharger le dataset si besoin
